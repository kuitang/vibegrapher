...
class TestMessageSending:
    ...
    async def test_send_message_to_session(self, test_client, test_project):
        ...
        session_response = await test_client.post(
            f"/projects/{test_project['id']}/sessions"
        )
...
class TestSessionDeletion:
    ...
    async def test_delete_session(self, test_client, test_project):
        ...
        await test_client.post(
            f"/sessions/{session['id']}/messages", json={"prompt": "Test message"}
        )
...
class TestSocketIOIntegration:
    ...
    async def test_socket_streaming_token_usage(
        self, test_client, socket_client, test_project
    ):
        ...
        await test_client.post(
            f"/sessions/{session['id']}/messages", json={"prompt": "Add a TODO comment"}
        )
    ...
    async def test_socket_conversation_message(
        self, test_client, socket_client, test_project
    ):
...
class TestSessionPersistence:
    ...
    async def test_openai_session_key_set(self, test_client, test_project):
        ...
        await test_client.post(
            f"/sessions/{session['id']}/messages", json={"prompt": "Initialize session"}
        )
