...
class TestDiffRetrieval:
    ...
    async def test_get_pending_diffs(self, test_client, test_session, test_diff):
        ...
        response = await test_client.get(f"/sessions/{test_session['id']}/diffs/pending")
        assert response.status_code == 200
...
class TestDiffCommit:
    ...
    async def test_commit_with_base_mismatch(self, test_client, test_diff, test_project):
        ...
        from app.services.git_service import GitService
        git_service = GitService()
        git_service.write_code(test_project["slug"], "# Changed\ndef hello():\n    return 'changed'")
...
class TestPageRefreshRecovery:
    ...
    async def test_recover_pending_diffs(self, test_client, test_session, test_diff):
        ...
        review_response = await test_client.post(
            f"/diffs/{pending_diff['id']}/review",
            json={"approved": True}
        )
