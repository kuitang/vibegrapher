...
async def test_evaluator_iteration(test_server: dict, caplog: Any) -> None:
    ...
    async with httpx.AsyncClient(base_url=test_server["url"], timeout=60.0) as client:
        ...
        message_resp = await client.post(
            f"/sessions/{session['id']}/messages",
            json={"prompt": "Add a new SpanishAgent class that translates messages to Spanish"}
        )
        assert message_resp.status_code == 200
        result = message_resp.json()
...
async def test_diff_creation_flow(test_server: dict) -> None:
    ...
    async with httpx.AsyncClient(base_url=test_server["url"], timeout=30.0) as client:
        ...
        project_resp = await client.post("/projects", json={"name": "Diff Test Project"})
        assert project_resp.status_code == 201
        project = project_resp.json()
        ...
        message_resp = await client.post(
            f"/sessions/{session['id']}/messages",
            json={"prompt": "Add a comment saying 'Hello World' at the end of the file"}
        )
        assert message_resp.status_code == 200
        result = message_resp.json()
        ...
        assert diff.get("patch"), "Should have patch content"
...
def test_real_openai_api_key_required() -> None:
    ...
    if not api_key:
        pytest.skip("OPENAI_API_KEY not set - this is expected if not running full tests")
